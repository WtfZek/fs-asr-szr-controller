    ...
    if(asrmodel=="2pass-offline" || asrmodel=="offline")
    {
        ...
        fetch('http://192.168.3.100:8018/human', {
            body: JSON.stringify({
                ...
                type: 'chat',
                interrupt: false,
                sessionid: parseInt(sessionId), // 使用获取到的sessionid值
            }),
            headers: {
                'Content-Type': 'application/json'
            },
...
window.onload = function() {
    ...
    if (speakerToggle) {
        speakerToggle.addEventListener('change', function() {
            // 根据开关状态设置阈值
            const thresholdValue = this.checked ? 1 : 100;
            document.getElementById('threshold-slider').value = thresholdValue;
            document.getElementById('threshold-value-display').textContent = thresholdValue;
